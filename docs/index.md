...
!!! warning "End of life alert"
    ...
    Percona XtraBackup 8.0 has reached End of Life (EOL). The core documentation is now read-only and is no longer updated with new features, enhancements, or bug fixes.

!!! note ""

    This documentation is for the latest release: [Percona XtraBackup {{update}}](release-notes/8.0/{{release}}.0.md) release notes.
...
Percona XtraBackup {{release}} can take backups of MySQL-compatible databases running earlier releases within the same major version, if specific conditions are met. [See the supported versions](supported-versions.md) for more details.
