...
For incremental backups with many InnoDB data (IBD) files, you can reduce prepare time by using the `--parallel` option. `--parallel` enables Percona XtraBackup to process multiple delta files concurrently and use available storage bandwidth more effectively.

The option is especially useful when a backup contains many IBD files. Each worker processes one file at a time, so Percona XtraBackup can process multiple `.delta` files concurrently. A single `.delta` file uses one worker.
