...
Certain versions of `libcurl` are known to cause unexpected terminations or connection reuse issues in `xbcloud`. The following versions are not supported.

### Unsupported versions

`libcurl 7.64.0 ` the default curl version in Debian 10
...
If the installed `libcurl` version isnot supported, update it depending on the operating system and available repositories.
